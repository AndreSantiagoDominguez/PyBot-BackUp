	...
	for _, table := range tables {
		...
		case "readings":
			...
			for _, r := range readings {
				if err = h.postgres.InsertReading(r); err != nil {
		...
		case "waste_collection":
			...
			for _, w := range wastes {
				if _, err = h.postgres.InsertWasteCollectionRegister(w); err != nil {
		...
		case "weight_data":
			...
			for _, w := range weights {
				if _, err = h.postgres.InsertWeightData(w); err != nil {
		...
		case "gps_data":
			...
			for _, g := range gpsList {
				if _, err = h.postgres.InsertGPSData(g); err != nil {
